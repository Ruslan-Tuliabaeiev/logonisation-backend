...
export class ArticleService {
    ...
    async findOne({id}: FindOneArticleDto): Promise<ArticleEntity | null> { // проверить правильность поиска
        const findOneArticle = this.articleEntityRepository.findOne({where:{id}})
        if (!findOneArticle) {
            throw new HttpException('Article not found', HttpStatus.NOT_FOUND);
        }
        return findOneArticle
    ...
    async update({id, article }: UpdateArticleDto): Promise<ArticleEntity | undefined> {
